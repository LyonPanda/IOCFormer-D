...
def validate(Pre_data, model, criterion, logger, args):
    ...
    for i, (fname, img, depth_img, kpoint, targets, patch_info) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            outputs = model(img, depth_img) #outputs:list, len=2
        ...
        for k in range(topk_values.shape[0]):
            ...
            sub_count[sub_count < args['threshold']] = 0 #args['threshold']=0.35
            sub_count[sub_count > 0] = 1
            sub_count = torch.sum(sub_count).item()
            count += sub_count

        if args['dm_count']: #Ture
            count_dm=0
        ...
        if args['visual_path']: #args['visual_path']=''
            # import pdb; pdb.set_trace()
            ...
            if False:
                ...
                res1 = np.hstack((ori_img_ori[200:1280, 128:2048,:], kpoint_map[200:1280, 128:2048,:]))
                res2 = np.hstack((density_map[200:1280, 128:2048,:], frame[200:1280, 128:2048,:]))
