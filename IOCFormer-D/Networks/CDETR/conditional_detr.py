...
class ConditionalDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, depth):
        ...
        if self.dm_decoder is not None:
            if self.branch_merge and self.branch_merge_way==2:
                if self.transformer_flag=="merge" or self.transformer_flag=="merge2" or self.transformer_flag=="merge3":
                    hs, reference = self.transformer(src, depth, mask, self.query_embed.weight, pos[-1], out_density)
                # else:
...
def build(args):
    ...
    num_classes = 2 if args.dataset_file != 'coco' else 91 #num_classes=2
    ...
    model = ConditionalDETR(
        ...
        num_queries=args.num_queries, #700
        channel_point = args.channel_point, #3
        aux_loss=args.aux_loss, #True
        dm_decoder=density_decoder,
        branch_merge=args.branch_merge, #True
        branch_merge_way=args.branch_merge_way, #2
        transformer_flag=args.transformer_flag #'merge3'
    )
    #False
    if args.masks:
    ...
    if args.aux_loss:
        aux_weight_dict = {}
        for i in range(args.dec_layers - 1): #args.dec_layers=6
