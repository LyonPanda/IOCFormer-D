...
if args.backbone == 'resnet50' or args.backbone == 'resnet101': #args.backbone == 'resnet50'
...
def main(args):
    ...
    eval_epoch = 0
    for epoch in range(args['start_epoch'], args['epochs'] + 1): #range(0, 1501)
        train(train_data, model, criterion, optimizer, epoch, scheduler, logger, writer, args, dm_losses)

        '''inference '''
        if epoch % args['test_per_epoch'] == 0 and epoch >= 1200:
...
def collate_wrapper(batch):
    ...
    for item in batch:
        ...
        for i in range(0, len(item[2])):
            depth_imgs.append(item[2][i])
            
        for i in range(0, len(item[3])):
            targets.append(item[3][i])

    return fname, torch.stack(imgs, 0), torch.stack(depth_imgs, 0), targets
...
def train(Pre_data, model, criterion, optimizer, epoch, scheduler, logger, writer, args, dm_losses):
    ...
    train_data = dataset.listDataset(Pre_data, args['save_path'],
                                     ...
                                     transform_depth=transforms.ToTensor(),
                                     train=True,
                                     args=args)
    ...
    for i, (fname, img, depth_img, targets) in enumerate(train_loader):
        img = img.cuda()
        depth_img.cuda()
        
        # import pdb; pdb.set_trace()
        N=img.shape[0]
        out_all=model(img, depth_img)
        if isinstance(out_all, list):
            ...
            ot_loss=ot_loss*wot #wot=0, ot_loss=0
            ...
            down_h=gt_discrete.shape[-2]//downsample_ratio #down_h=256//8=32
            down_w=gt_discrete.shape[-1]//downsample_ratio #down_w=256//8=32
            ...
            tv_loss = (dm_losses[1](out_dm[2], gt_discrete_normed).sum(1).sum(1).sum(1) * torch.from_numpy(gd_count).float().cuda()).mean(0) * wtv #wtv=0, tv_loss=0
...
def validate(Pre_data, model, criterion, epoch, logger, args):
    ...
    for i, (fname, img, depth_img, kpoint, targets, patch_info) in enumerate(test_loader):
