...
import os
from image import load_data, load_depth_data
import random
...
def gen_discrete_map(im_height, im_width, points):
    ...
    discrete_map = torch.zeros(im_width * im_height).scatter_add_(0, index=p_index.long(), src=torch.ones(im_width*im_height)).view(im_height, im_width).numpy() #在windows上加了.long()
...
class listDataset(Dataset):
    def __init__(self, root, shape=None, shuffle=True, transform=None, transform_depth=None, train=False, seen=0, batch_size=1,
    ...
    def __getitem__(self, index):
        ...
        depth_img_path = img_path.replace('images_2048','depthmaps_2048')
        fname = os.path.basename(img_path)
        img, kpoint = load_data(img_path, self.args, self.train)
        depth_img = load_depth_data(depth_img_path, self.args, self.train)
        while min(kpoint.shape[0], kpoint.shape[1]) < self.args['crop_size']  and self.train == True:
            random_index = random.randint(1, self.nSamples-1)
            img_path = self.lines[random_index]
            depth_img_path = img_path.replace('images_2048','depthmaps_2048')
            fname = os.path.basename(img_path)
            img, kpoint = load_data(img_path, self.args, self.train)
            depth_img = load_depth_data(depth_img_path, self.args, self.train)
        ...
        if self.train == True:
            ...
            if self.args['scale_aug'] == True and random.random() > (1 - self.args['scale_p']): # random scale
                ...
                elif self.args['scale_type'] == 1: # self.args['scale_type'] == 1
                ...
                if min(width, height) > self.args['crop_size']:
                    ...
                    depth_img = depth_img.resize((width, height), Image.ANTIALIAS)
                else:
                    self.rate = 1
            else:
        ...
        if self.train == True:
            ...
            for l in range(self.args['num_patch']):
                while True:
                    ...
                    width = self.args['crop_size'] #256
                    height = self.args['crop_size'] #256
                    try:
                    ...
                    sub_depth_img = depth_img[:, crop_size_x: crop_size_x + width, crop_size_y:crop_size_y + height]
                    '''crop kpoint'''
                    ...
                    distances = self.caculate_knn_distance(gt_points, num_points) # distances.shape:[num_points, 1]
                    points = torch.cat([gt_points, distances], dim=1)

                    #points = gt_points
                    # added by gls
                    gt_points_scale=gt_points/float(width)*self.args['crop_size'] #width=self.args['crop_size'],gt_points_scale=gt_points
                ...
                imgs.append(sub_img)
                depth_imgs.append(sub_depth_img)
                targets.append(target)

            return fname, imgs, depth_imgs, targets
            ...
            depth_img = F.pad(depth_img, pd, 'constant')
            ...
            depth_img_return = depth_img.view(1, num_h, self.args['crop_size'], width).view(1, num_h, self.args['crop_size'], num_w,
                                                                                   self.args['crop_size'])
            depth_img_return = depth_img_return.permute(0, 1, 3, 2, 4).contiguous().view(1, num_w * num_h, self.args['crop_size'],
                                                                                        self.args['crop_size']).permute(1, 0, 2, 3)
            ...
            return fname, img_return, depth_img_return, kpoint_return, targets, patch_info
